...
**Result:** Lightspeed thinks it's talking to OpenAI, but it's actually using Gemini. 🚀
...
LiteLLM Proxy (Pod in cluster)
    ↓
    │ (Translated to Google Gemini format)
    ↓
...
- **`litellm.yaml`** — Kubernetes deployment for LiteLLM proxy
- **`ols-config.yaml`** — OpenShift Lightspeed configuration
